...
class TVB_test:
	...
	def setup_params(self, nc, ns):  # {{{
		...
		coupling_file = open('/p/project/cslns/wikicollab/Parsweep_L2L_RateML/couplings_%d' % self.args.procid, 'rb')
		couplings = pickle.load(coupling_file)
		coupling_file.close()

		speed_file = open('/p/project/cslns/wikicollab/Parsweep_L2L_RateML/speeds_%d' % self.args.procid, 'rb')
		...
		# couplings = []
		# speeds = []
		# for cp in (couplingsL2L):
		# 	couplings.append(cp[0])
		# for sp in (speedsL2L):
		# 	speeds.append(sp[0])

		# Params tuned for G2DO exploration
		# couplings = np.linspace(0.003, 0.005, nc)
		# speeds = np.linspace(3.0, 5.0, ns)

		# print('couplings', couplings)
		# print('speeds', speeds)

		# For comparing to single tvb; setting all params equal
		#couplings = np.array([.0042]*32)
		#speeds = np.array([4.0]*32)
	...
	def startsim(self):
		...
		print('ccFCSC', ccFCSC)
		resL2L_file = open('/p/project/cslns/vandervlag1/L2L/results/result_%d' % self.args.procid, 'wb')
